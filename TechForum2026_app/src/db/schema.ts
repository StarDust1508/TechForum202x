...
import { pgTable, text, integer, bigint, boolean, timestamp, primaryKey, index, varchar, jsonb } from 'drizzle-orm/pg-core';
...
export const sessionsEvent = pgTable(
  'sessions_event',
  {
    ...
    // Public visibility is independent from programme content. Only the
    // dedicated CAS endpoint may change these fields.
    isPublished: boolean('is_published').notNull().default(false),
    publicationVersion: bigint('publication_version', { mode: 'number' }).notNull().default(0),
    ...
    publicationReportedActor: text('publication_reported_actor'),
  },
  (t) => ({
    ...
    publicationIdx: index('sessions_publication_idx').on(t.isPublished, t.dayId, t.startTime),
  }),
);

export const programmeSessionPublicationRevisions = pgTable(
  'programme_session_publication_revisions',
  {
    id: text('id').primaryKey(),
    requestId: text('request_id').notNull(),
    sessionId: text('session_id').notNull().references(() => sessionsEvent.id, { onDelete: 'restrict' }),
    actor: text('actor').notNull(),
    reportedActor: text('reported_actor'),
    reason: text('reason').notNull(),
    beforeIsPublished: boolean('before_is_published').notNull(),
    afterIsPublished: boolean('after_is_published').notNull(),
    beforeVersion: bigint('before_version', { mode: 'number' }).notNull(),
    afterVersion: bigint('after_version', { mode: 'number' }).notNull(),
    beforePayload: jsonb('before_payload').notNull(),
    afterPayload: jsonb('after_payload').notNull(),
    createdAt: timestamp('created_at', { withTimezone: true }).notNull().defaultNow(),
  },
  (t) => ({
    requestSessionIdx: index('programme_session_publication_revisions_request_session_idx').on(t.requestId, t.sessionId),
    sessionCreatedIdx: index('programme_session_publication_revisions_session_created_idx').on(t.sessionId, t.createdAt),
  }),
);

export const programmeSessionPublicationRuns = pgTable('programme_session_publication_runs', {
  runId: text('run_id').primaryKey(),
  mode: text('mode').notNull(),
  targetSessionIds: text('target_session_ids').array().notNull(),
  actor: text('actor').notNull(),
  reportedActor: text('reported_actor'),
  reason: text('reason').notNull(),
  beforeTotal: integer('before_total').notNull(),
  beforePublic: integer('before_public').notNull(),
  afterTotal: integer('after_total').notNull(),
  afterPublic: integer('after_public').notNull(),
  affectedCount: integer('affected_count').notNull(),
  auditCount: integer('audit_count').notNull(),
  beforeProgrammeSha256: text('before_programme_sha256').notNull(),
  afterProgrammeSha256: text('after_programme_sha256').notNull(),
  beforeRegistrationCount: integer('before_registration_count').notNull(),
  afterRegistrationCount: integer('after_registration_count').notNull(),
  preservedRegistrationCount: integer('preserved_registration_count').notNull(),
  beforeRegistrationsSha256: text('before_registrations_sha256').notNull(),
  afterRegistrationsSha256: text('after_registrations_sha256').notNull(),
  beforeStateSha256: text('before_state_sha256').notNull(),
  afterStateSha256: text('after_state_sha256').notNull(),
  createdAt: timestamp('created_at', { withTimezone: true }).notNull().defaultNow(),
});
