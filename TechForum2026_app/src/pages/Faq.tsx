...
export default function Faq() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const r = await fetch(resolveApiUrl('/faq'), { credentials: 'include' });
        if (r.ok) {
    ...
    <PageShell kicker="Помощь" title="Гид" subtitle="Ответы на частые вопросы">
      ...
      {loading && items.length === 0 && (
        <div className="space-y-3">
          {[0, 1, 2, 3].map((i) => (
            <div key={i} className="rounded-2xl border border-[#4ec9c0]/22 bg-[#0a2f38]/40 p-5 space-y-2">
      ...
      {!loading && filtered.length === 0 && (
        <div className="rounded-3xl border border-dashed border-[#4ec9c0]/25 bg-[#0a2f38]/30 p-8 text-center">
          <HelpCircle className="w-9 h-9 mx-auto text-[#4ec9c0]/60" strokeWidth={1.4} />
          ...
          <p className="mt-1 text-[12px] text-[#7aa8a4]">
            Не нашли ответ? Напишите на support@techforum.ru
          </p>
        </div>
      )}
              ...
              {list.map((f, idx) => {
                ...
                return (
                  <motion.article
                    ...
                    className="rounded-2xl border border-[#4ec9c0]/22 bg-[#0a2f38]/40 overflow-hidden"
                  >
                    <button
                      type="button"
                    ...
                    >
                      <span className="font-display-cyrl text-[14px] font-semibold text-[#d8f0ee] leading-snug">
                        {f.question}
                      </span>
                      <motion.span
                        animate={{ rotate: isOpen ? 180 : 0 }}
                        transition={{ duration: 0.2 }}
                        className="shrink-0 text-[#4ec9c0]"
                      >
                    ...
                    <AnimatePresence initial={false}>
                      {isOpen && (
                        ...
                        >
                          <div className="px-5 pb-4 text-[13px] text-[#d8f0ee]/80 leading-relaxed border-t border-[#4ec9c0]/15 pt-3">
                            {f.answer}
                          </div>
                        </motion.div>
                      )}
