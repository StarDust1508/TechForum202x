  ...
  return (
    <div className="pb-24 pt-6 px-5 space-y-8" style={{ paddingTop: 'calc(env(safe-area-inset-top, 0px) + 64px)' }}>
      ...
      <div className="space-y-6">
        ...
        <div className="aspect-[4/5] bg-card border border-card-border rounded-[3rem] relative overflow-hidden flex items-center justify-center group">
          <img 
          ...
          <div className="relative z-10 text-center space-y-4">
            ...
            <div>
              {/* ГПС координаты */}
              <p className="text-[8px] text-muted uppercase mt-1 tracking-widest">ГПС: 55.7558° С, 37.6173° В</p>
            </div>
          </div>
        </div>
        ...
        <section className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              ...
              <div key={i} className="bg-card border border-card-border p-4 rounded-[2rem] flex items-center gap-3 hover:border-accent/40 transition-colors">
                <div className="w-10 h-10 bg-accent/10 rounded-2xl flex items-center justify-center text-accent">
                  <item.icon className="w-5 h-5" />
                </div>
                <div className="min-w-0">
                  <div className="text-xs font-bold text-primary truncate">{item.label}</div>
        ...
        <section className="bg-accent/10 border border-accent/20 p-5 rounded-[2.5rem] flex items-start gap-4">
          <div className="w-12 h-12 bg-accent rounded-2xl flex items-center justify-center text-surface shrink-0">
            <Info className="w-6 h-6" />
          </div>
          <div className="space-y-1">
            <h4 className="font-bold text-primary text-sm">Центральная регистрация</h4>
            <p className="text-xs text-muted leading-relaxed font-medium">Расположена у главного входа. Не забудьте получить бейджи и стартовый пакет участника до 10:00.</p>
          </div>
        </section>
      </div>
    </div>
