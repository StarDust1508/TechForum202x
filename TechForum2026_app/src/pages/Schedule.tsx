        ...
        <div className="px-4 pb-4 pt-3 min-[360px]:px-5"><div className="flex items-start gap-3"><BackButton /><div className="min-w-0"><h1 className="font-display text-[clamp(25px,7vw,30px)] font-bold leading-none">Программа</h1><p className="mt-2 text-[13px] leading-snug text-foreground/60">Выберите день и тему. Добавляйте выступления в личный план.</p></div></div>
          ...
          {selectedDay === MY ? (
            ...
            <>
              ...
              <div className="-mx-4 mt-3 flex gap-2 overflow-x-auto px-4 pb-1 scrollbar-hide min-[360px]:-mx-5 min-[360px]:px-5" aria-label="Темы программы"><button type="button" aria-pressed={selectedTrack === 'all'} onClick={() => setSelectedTrack('all')} className={`min-h-11 min-w-11 rounded-xl border px-3 py-2 text-[13px] font-bold ${selectedTrack === 'all' ? 'border-primary bg-primary text-primary-foreground' : 'border-border bg-card text-foreground/65'}`}>Все темы</button>{tracks.map((track) => <button type="button" key={track.id} aria-pressed={selectedTrack === track.id} onClick={() => setSelectedTrack(track.id)} className={`min-h-11 min-w-11 rounded-xl border px-3 py-2 text-[13px] font-bold whitespace-nowrap ${selectedTrack === track.id ? 'border-primary bg-primary text-primary-foreground' : 'border-border bg-card text-foreground/65'}`}>{track.shortLabel}</button>)}</div>
            </>
          )}
        </div>
      </header>
