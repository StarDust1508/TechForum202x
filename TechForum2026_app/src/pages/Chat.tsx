...
import { 
  ...
  ChevronRight, History, Image, Video, Mic, 
  X, Play, Pause, Paperclip, Camera, Radio
} from 'lucide-react';
...
export default function Chat() {
  ...
  const [contacts] = useState([
    { id: '1', name: 'Алексей', role: 'Fullstack Developer', avatar: 'А', online: true, lastMsg: 'Договорились, встретимся у стенда.' },
    { id: '2', name: 'Мария', role: 'Product Designer', avatar: 'М', online: false, lastMsg: 'Выслала обновленные макеты в почту.' },
    { id: '3', name: 'Игорь', role: 'Tech Investor', avatar: 'И', online: true, lastMsg: 'Очень интересное выступление!' },
  ]);
  ...
  // Multimedia component for messages
  const MediaMessage = ({ media, role }: { media: NonNullable<ChatMessage['media']>, role: ChatMessage['role'] }) => {
    ...
    if (media.type === 'video') {
      return (
        <div className="relative rounded-[1.5rem] overflow-hidden border border-white/10 aspect-video bg-black/40 group mt-2">
          <video src={media.url} className="w-full h-full object-cover" />
          <div className="absolute inset-0 flex items-center justify-center group-hover:bg-black/20 transition-all">
            <button className="w-14 h-14 bg-accent/90 rounded-full flex items-center justify-center shadow-2xl scale-90 group-hover:scale-100 transition-transform">
              <Play className="w-7 h-7 text-surface ml-1" />
            </button>
          </div>
        </div>
      );
  ...
  const sendMultimediaMessage = (type: any, url: string) => {
  ...
  return (
    <div className="flex flex-col h-full bg-surface relative">
      ...
      <div className="flex-1 overflow-y-auto p-5 scrollbar-hide pb-28">
        {activeTab === 'Ассистент' ? (
            ...
            {messages.map((msg, i) => (
              ...
              >
                ...
                <div className="space-y-0.5 max-w-full">
                  <div className={cn(
                    "px-4 py-3 text-[14px] leading-snug shadow-sm transition-all animate-in fade-in slide-in-from-bottom-2",
                    msg.role === 'user' 
          ...
          <div className="flex flex-col gap-4">
            {[
              { user: 'Алексей', text: 'Всем привет! Кто уже на главной сцене? Там Дмитрий Волков начинает через 10 минут.', time: '09:50', avatar: 'А', id: '1' },
              { user: 'Мария', text: 'Я тут! Очередь на регистрацию была огромная, но уже внутри.', time: '09:52', avatar: 'М', id: '2' },
              { user: 'Игорь', text: 'Ребята, какой пароль от Wi-Fi в Зале А?', time: '09:55', avatar: 'И', id: '3' },
              { user: 'Организатор', text: 'Пароль от Wi-Fi: TF2026_Guest. Приятного форума!', time: '09:56', avatar: 'O', isStaff: true },
              { user: 'Елена', text: 'Спикер в Зале Б просто огонь! Всем советую заглянуть после кофе-брейка.', time: '10:15', avatar: 'Е' },
            ].map((msg, i) => (
              <div key={i} className="flex gap-4 group">
                <button 
                  ...
                  className={cn(
                    "w-10 h-10 rounded-2xl flex items-center justify-center text-sm font-black shrink-0 border transition-transform active:scale-90",
                ...
                <div className="space-y-1.5 flex-1 min-w-0">
                  ...
                  <div className="bg-card/40 border border-card-border p-4 rounded-3xl rounded-tl-none text-[13px] leading-relaxed text-primary/80 group-hover:border-accent/20 transition-colors">
                    {msg.text}
                  </div>
                </div>
              </div>
            ...
            {!selectedContact ? (
                ...
                {contacts.map((contact) => (
                  ...
                  >
                    <div className="relative">
                      <div className="w-12 h-12 bg-card border border-card-border rounded-2xl flex items-center justify-center text-lg font-black text-primary">
                        {contact.avatar}
                      </div>
                      {contact.online && (
                    ...
                    <div className="flex-1 text-left min-w-0">
                      <div className="flex justify-between items-center mb-0.5">
                        ...
                        <span className="text-[9px] text-muted">{contact.lastMsg.includes('Сегодня') ? '' : '15:30'}</span>
                      </div>
              ...
              <div className="space-y-6 min-h-full flex flex-col pt-2">
                <button 
                  ...
                  className="flex items-center gap-2 mb-4 text-accent text-[10px] font-black uppercase tracking-widest hover:opacity-70 transition-opacity"
                >
                ...
                <div className="flex-1 flex flex-col gap-2 pt-2">
                  {(privateMessages[selectedContact] || []).map((msg, i) => (
                    ...
                    >
                      <div className={cn(
                        "w-9 h-9 rounded-2xl flex items-center justify-center shrink-0 border",
                        msg.role === 'user' ? "bg-accent/20 border-accent/30 text-accent font-black" : "bg-card border-card-border text-muted font-black"
                      )}>
                        {msg.role === 'user' ? 'Я' : contacts.find(c => c.id === selectedContact)?.avatar}
                      </div>
      ...
      {activeTab !== 'История' && (activeTab !== 'Личные' || selectedContact) && (
        <div className="sticky bottom-0 left-0 right-0 p-4 bg-[#0f1218] border-t border-card-border z-40 mt-auto">
          <AnimatePresence>
            {mediaPreview && (
              <motion.div 
                ...
                className="mb-4 relative rounded-3xl overflow-hidden border-2 border-accent/40 bg-card p-2"
              >
                <button 
            ...
            {isRecording && (
              <motion.div 
                ...
                exit={{ opacity: 0, y: 20 }}
                className="mb-4 bg-accent/10 border border-accent/30 p-4 rounded-3xl flex items-center justify-between"
              >
                <div className="flex items-center gap-3">
                  <div className="w-2.5 h-2.5 bg-red-500 rounded-full animate-pulse" />
                  <span className="text-xs font-black text-accent uppercase tracking-widest leading-none">
                    Запись {isRecording === 'video' ? 'видео' : 'аудио'}... {recordingTime}с
                  </span>
                </div>
                <button 
                  onClick={stopMediaRecording}
                  className="bg-accent text-surface px-4 py-2 rounded-xl text-[10px] font-black uppercase"
                >
                  Готово
                </button>
              </motion.div>
          ...
          <div className="flex items-center gap-2">
            ...
            <textarea
              ...
              placeholder={
                ...
                activeTab === 'Личные' ? `Написать ${contacts.find(c => c.id === selectedContact)?.name}...` :
                "Спросить ассистента..."
              }
              rows={1}
              className="flex-1 bg-[#1a1e2a] border border-card-border rounded-[1.5rem] px-5 py-3 text-sm focus:outline-none focus:border-accent/40 text-primary placeholder:text-muted/60 resize-none min-h-[44px]"
            />
            ...
            {!input.trim() ? (
              <div className="flex gap-2">
                <button
                  ...
                  className="w-11 h-11 bg-card border border-card-border rounded-[1.25rem] flex items-center justify-center text-muted hover:text-accent transition-colors active:scale-95 translate-y-0.5"
                >
              ...
              <button
                onClick={handleSend}
                disabled={loading}
                className="w-11 h-11 bg-accent rounded-[1.25rem] flex items-center justify-center text-surface disabled:opacity-30 disabled:grayscale transition-all hover:scale-105 active:scale-95 shadow-lg shadow-accent/20"
              >
